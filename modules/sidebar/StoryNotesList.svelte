<script>
  ...
  function relMtime(mtime) {
    ...
    return new Date(mtime * 1000).toLocaleDateString("fr-CA");
  }
</script>

<div class="notes-list">
  <section class="group">
    <h3 class="group-title">Épinglées</h3>
    {#if pinned.length === 0}
      <p class="empty">Aucune histoire épinglée</p>
    {:else}
      <ul>
        {#each pinned as note, i (note.notePath)}
          <li
            ...
            role="button"
            tabindex="0"
            onclick={() => onOpen(note.folderPath)}
            onkeydown={(e) => e.key === "Enter" && onOpen(note.folderPath)}
          >
            ...
            <button
              class="unpin"
              title="Désépingler"
              onclick={(e) => { e.stopPropagation(); onUnpin(note.notePath); }}
            >×</button>
          </li>
        {/each}
      </ul>
  ...
  <section class="group">
    <h3 class="group-title">Récentes</h3>
    {#if recent.length === 0}
      <p class="empty">Aucune histoire récente</p>
    {:else}
      <ul>
          ...
          >
            ...
            {#if !note.pinned}
              <button
                class="pin"
                title="Épingler"
                onclick={(e) => { e.stopPropagation(); onPin(note.notePath); }}
              >＋</button>
            {/if}
          </li>
        {/each}
